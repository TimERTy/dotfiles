...
notify_user() {
    local summary="" message="" time="3000"
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --a|--i) shift 2 ;;
            --s) summary="$2"; shift 2 ;;
    ...
    notify-send -a "$APP_NAME" -i "$NOTIFICATION_ICON" "$summary" "$message" -t "$time"
}
...
take_instant_full() {
    ...
    local out="$screenshot_folder/$NAME"
    grim "$out" && notify_user \
        --a "${APP_NAME}" \
        --i "${NOTIFICATION_ICON}" \
        --s "Screenshot saved" \
        --m "$out" \
...
takescreenshot() {
    sleep 1
    mkdir -p "$screenshot_folder"
    grimblast --notify "$option_chosen" "$option_type_screenshot" "$screenshot_folder/$NAME"
}
